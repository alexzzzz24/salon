...
MAIN_MENU(){
  ...
  AVAILABLE_SERVICES=$($PSQL "SELECT service_id, name FROM services") #query info from the table
  if [[ -z $AVAILABLE_SERVICES ]] #check if there is any info in the table
  then #if no
   echo "We don't have this service"
  else #if yes 
   echo "$AVAILABLE_SERVICES" | while read SERVICE_ID BAR NAME #read from the table
   ...
   then
   MAIN_MENU "Please pick a service"
   else 
